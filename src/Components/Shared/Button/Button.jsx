...
export const ButtonFill = ({ children, onClick, className }) => {
  return (
    <button
      onClick={onClick}
      className={`px-[25px] py-2 rounded-[5px] text-[10px] md:text-[12px] lg:text-[18px] capitalize font-medium text-white bg-primary hover:bg-primary-500 transition duration-300 ease-out ${className}`}
    >
      {children}
    </button>
  );
